        ...
        match *irrefutable_pat.kind {
            ...
            PatternKind::AscribeUserType {
                ...
                let user_ty = box pat_ascription_ty.user_ty(
                    ...
                    place.ty(&self.local_decls, self.hir.tcx()).to_ty(),
                    ty_source_info.span,
                );
                self.cfg.push(
                    block,
...
impl<'a, 'gcx, 'tcx> Builder<'a, 'gcx, 'tcx> {
    ...
    fn calculate_fake_borrows<'b>(
        ...
        all_fake_borrows.into_iter().map(|matched_place| {
            let fake_borrow_deref_ty = matched_place.ty(&self.local_decls, tcx).to_ty();
    ...
    fn ascribe_types<'pat>(
        ...
        for ascription in ascriptions {
            ...
            let user_ty = box ascription.user_ty.clone().user_ty(
                &mut self.canonical_user_type_annotations,
                ascription.source.ty(&self.local_decls, self.hir.tcx()).to_ty(),
                source_info.span
            );
            self.cfg.push(
                block,
