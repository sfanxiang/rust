            ...
            Rvalue::Ref(_, _, ref place) => {
                ...
                if let Place::Projection(ref proj) = *place {
                    if let ProjectionElem::Deref = proj.elem {
                        let base_ty = proj.base.ty(cx.mir, cx.tcx).to_ty();
    ...
    fn in_rvalue(cx: &ConstCx<'_, 'tcx>, rvalue: &Rvalue<'tcx>) -> bool {
        match *rvalue {
            ...
            Rvalue::Ref(_, kind, ref place) => {
                let ty = place.ty(cx.mir, cx.tcx).to_ty();
            ...
            Place::Projection(ref proj) => {
                match proj.elem {
                    ProjectionElem::Deref => {
                        ...
                        let base_ty = proj.base.ty(self.mir, self.tcx).to_ty();
                        match self.mode {
                            Mode::Fn => {},
                            _ => {
        ...
        if let TerminatorKind::Call { ref func, ref args, ref destination, .. } = *kind {
            if let Some((ref dest, _)) = *destination {
                self.assign(dest, ValueSource::Call {
                    callee: func,
                    args,
                    return_ty: dest.ty(self.mir, self.tcx).to_ty(),
                }, location);
            }
            ...
            if self.mode != Mode::Fn {
                ...
                if let Some(span) = needs_drop {
                    ...
                    let ty = place.ty(self.mir, self.tcx).to_ty();
